...
BOOL CCSGDesitnationCombo::OnIdle(LONG lCount) {
  ...
  if ((m_pLastDoc != pDoc) || ((pDoc == NULL) && (m_pLastDoc != NULL))
    ...
    if (iSelectedEntryByPtr != -1)
      SetCurSel(iSelectedEntryByPtr);
    else if (iSelectedEntryByName != -1)
      SetCurSel(iSelectedEntryByName);
    else
      SetCurSel(0);
