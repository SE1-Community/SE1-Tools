...
CDlgWorldSettings::CDlgWorldSettings(CWnd *pParent /*=NULL*/) : CDialog(CDlgWorldSettings::IDD, pParent) {
...
void CDlgWorldSettings::DoDataExchange(CDataExchange *pDX) {
  ...
  if (pDX->m_bSaveAndValidate != FALSE) {
    ...
    if (m_fnBackgroundPicture != "") {
      theApp.WriteProfileString(L"World editor prefs", L"Default background picture", m_fnBackgroundPicture);
    }
...
void CDlgWorldSettings::OnBrowseBackgroundPicture() {
  CTFileName fnChoosedFile = _EngineGUI.FileRequester("Select background texture", FILTER_TEX FILTER_ALL FILTER_END,
                                                      KEY_NAME_BACKGROUND_TEXTURE_DIR, "Textures\\Background");
  if (fnChoosedFile == "")
    return;
...
void CDlgWorldSettings::OnBrowseTopViewPicture() {
  CTFileName fnPicture
    = _EngineGUI.FileRequester("Picture for top view", FILTER_PICTURES FILTER_ALL FILTER_END, "Picture for view directory", "");
  if (fnPicture == "")
    return;
...
void CDlgWorldSettings::OnBrowseFrontViewPicture() {
  CTFileName fnPicture
    = _EngineGUI.FileRequester("Picture for front view", FILTER_PICTURES FILTER_ALL FILTER_END, "Picture for view directory", "");
  if (fnPicture == "")
    return;
...
void CDlgWorldSettings::OnBrowseRightViewPicture() {
  CTFileName fnPicture
    = _EngineGUI.FileRequester("Picture for right view", FILTER_PICTURES FILTER_ALL FILTER_END, "Picture for view directory", "");
  if (fnPicture == "")
    return;
...
void CDlgWorldSettings::OnBrowseBackdropObject() {
  CTFileName fnObject
    = _EngineGUI.FileRequester("Select background object", FILTER_3DOBJ FILTER_LWO FILTER_OBJ FILTER_3DS FILTER_ALL FILTER_END,
                               "Backdrop object directory", "");
  if (fnObject == "")
    return;
