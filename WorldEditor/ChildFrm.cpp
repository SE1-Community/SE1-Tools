...
BOOL CChildFrame::PreCreateWindow(CREATESTRUCT &cs) {
  ...
  if (((pFrame->MDIGetActive(&bMaximized) == NULL) || (bMaximized)) && (theApp.m_Preferences.ap_AutoMaximizeWindow)) {
...
void CChildFrame::KeyPressed(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  if (nFlags == 0x52)
    iViewConfiguration = 0;
  if (nFlags == 0x4f)
    iViewConfiguration = 1;
  if (nFlags == 0x50)
    iViewConfiguration = 2;
  if (nFlags == 0x51)
    iViewConfiguration = 3;
  if (nFlags == 0x4b)
    iViewConfiguration = 4;
  if (nFlags == 0x4c)
    iViewConfiguration = 5;
  if (nFlags == 0x4d)
    iViewConfiguration = 6;
  if (nFlags == 0x47)
    iViewConfiguration = 7;
  if (nFlags == 0x48)
    iViewConfiguration = 8;
  if (nFlags == 0x49)
    iViewConfiguration = 9;
...
CWorldEditorView *CChildFrame::GetPerspectiveView(void) {
  ...
  for (INDEX iVerticalSpliter = 0; iVerticalSpliter < GetVSplitters(); iVerticalSpliter++) {
    for (INDEX iHorizontalSpliter = 0; iHorizontalSpliter < GetHSplitters(); iHorizontalSpliter++) {
      CWorldEditorView *pWEDView = (CWorldEditorView *)m_wndSplitter.GetPane(iVerticalSpliter, iHorizontalSpliter);
      ASSERT(pWEDView != NULL);
...
void CChildFrame::TestGame(BOOL bFullScreen) {
  ...
  if ((!pDoc->m_bWasEverSaved && wed_bSaveTestGameFirstTime) || pDoc->IsModified()) { // save world under temporary name
    ASSERT_VALID(pDoc);
    try {
  ...
  if (bFullScreen) {
    ...
    hWndFullScreen = CreateWindowExA(WS_EX_TOPMOST, APPLICATION_NAME,
                                     "Serious Editor - Full Screen Test Game", // title
                                     WS_POPUP, 0, 0, pixSizeI, pixSizeJ,       // window size
                                     NULL, NULL, hInstanceFullScreen, NULL);
    // didn't make it?
