...
PIXaabbox2D CWndTerrainTilePalette::GetTileBBox(INDEX iTile) {
  PIXaabbox2D boxScr = PIXaabbox2D(
    PIX2D((iTile % m_ctPaletteTilesH) * PIX_TILE_WIDTH + 1, (iTile / m_ctPaletteTilesH) * PIX_TILE_HEIGHT + 1),
    PIX2D((iTile % m_ctPaletteTilesH + 1) * PIX_TILE_WIDTH + 1, (iTile / m_ctPaletteTilesH + 1) * PIX_TILE_HEIGHT + 1));
...
void CWndTerrainTilePalette::OnPaint() {
  ...
  if ((m_pDrawPort != NULL) && (m_pDrawPort->Lock())) {
    ...
    for (INDEX iTile = 0; iTile < m_dcTileInfo.Count(); iTile++) {
      ...
      MEXaabbox2D boxTex = MEXaabbox2D(MEX2D(ti.ti_ix * pixTileSize, ti.ti_iy * pixTileSize),
      ...
      m_pDrawPort->AddTexture(fI0, fJ0, fU0, fV0, col, fI1, fJ1, fU1, fV1, col, fI2, fJ2, fU2, fV2, col, fI3, fJ3, fU3, fV3, col);
    ...
    m_pDrawPort->DrawBorder(boxScr.Min()(1), boxScr.Min()(2), boxScr.Size()(1), boxScr.Size()(2), colSelected | CT_OPAQUE);
    ...
    for (INDEX itum = 0; itum < m_dcTileInfo.Count(); itum++) {
      ...
      if ((boxScr & boxPoint) == boxPoint) {
        ...
        m_pDrawPort->DrawBorder(boxScr.Min()(1), boxScr.Min()(2), boxScr.Size()(1), boxScr.Size()(2), C_BLUE | CT_OPAQUE,
                                ulLineType);
        break;
      }
    }
...
BOOL CWndTerrainTilePalette::Initialize(PIX pixX, PIX pixY, CTextureData *ptd, BOOL bCenter /*=TRUE*/) {
  ...
  if (IsWindow(m_hWnd)) {
    SetWindowPos(NULL, rectWindow.left, rectWindow.top, rectWindow.right - rectWindow.left, rectWindow.top - rectWindow.bottom,
                 SWP_NOZORDER | SWP_NOACTIVATE);
    ShowWindow(SW_SHOW);
  } else {
    // create window
    CMainFrame *pMainFrame = STATIC_DOWNCAST(CMainFrame, AfxGetMainWnd());
    BOOL bResult = CreateEx(WS_EX_TOOLWINDOW, NULL, L"Terrain tile palette", WS_CHILD | WS_POPUP | WS_VISIBLE, rectWindow.left,
...
void CWndTerrainTilePalette::OnTimer(UINT nIDEvent) {
  ...
  if (pt.x < rectWnd.left || pt.x > rectWnd.right || pt.y < rectWnd.top || pt.y > rectWnd.bottom) {
    DestroyWindow();
    DeleteTempMap();
    return;
  }
