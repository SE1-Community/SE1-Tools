...
void CDlgPgGlobal::DoDataExchange(CDataExchange* pDX) {
  ...
  if (pDX->m_bSaveAndValidate == FALSE) {
    ...
    if (theApp.m_ptdActiveTexture != NULL) {
      m_strTextureInfo = (CTString&)theApp.m_ptdActiveTexture->GetName() + " " + theApp.m_ptdActiveTexture->GetDescription();
    }
  }
...
BOOL CDlgPgGlobal::OnInitDialog() {
  ...
  m_wndActiveTexture.Create(NULL, NULL, WS_BORDER | WS_VISIBLE, CRect(pixLeft, pixTop, pixLeft + 96, pixTop + 96), this,
                            IDW_ACTIVE_TEXTURE);
