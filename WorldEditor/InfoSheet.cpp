...
CInfoSheet::CInfoSheet(CWnd* pWndParent) : CPropertySheet(AFX_IDS_APP_TITLE, pWndParent) {
  ...
  if (pDoc == NULL) {
    ...
    // else if we are in polygon mode
      ...
      if (m_ModeID == INFO_MODE_POLYGON) {
        if (GetActivePage() == &m_PgTexture)
          _iLastActivePgInPolygonMode = 1;
        else if (GetActivePage() == &m_PgShadow)
          _iLastActivePgInPolygonMode = 2;
        else
          _iLastActivePgInPolygonMode = 3;
      }
      if (m_ModeID != INFO_MODE_POLYGON) {
        SetInfoModePolygon();
      }
...
BOOL CInfoSheet::PreTranslateMessage(MSG* pMsg) {
  ...
  if (pMsg->message == WM_KEYDOWN) {
    ...
    if ((pDoc != NULL) && !bSectorNameTyping) {
      if ((pMsg->wParam == VK_ADD) || (pMsg->wParam == VK_SUBTRACT) || (pMsg->wParam == 'E') || (pMsg->wParam == 'S')
          || (pMsg->wParam == 'P') || (pMsg->wParam == 'Z') || (pMsg->wParam == 'Q') || (pMsg->wParam == VK_DECIMAL)) {
