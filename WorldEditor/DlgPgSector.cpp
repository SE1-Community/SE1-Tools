...
CDlgPgSector::CDlgPgSector() : CPropertyPage(CDlgPgSector::IDD) {
  ...
  if (CTString(CStringA(theApp.GetProfileString(L"World editor", L"Color browsing mode (info)"))) == CTString("RGB")) {
...
void CDlgPgSector::DoDataExchange(CDataExchange* pDX) {
  ...
  if ((pDX->m_bSaveAndValidate == FALSE) && IsWindow(m_SectorAmbientColor.m_hWnd)) {
    ...
    if (bSelectionExists) {
      ...
      FOREACHINDYNAMICCONTAINER(pDoc->m_selSectorSelection, CBrushSector, itbsc) {
        ...
        if (iForceField != iFirstForceField)
      ...
      if (bSameForceField)
        m_comboForceField.SetCurSel(iFirstForceField);
      if (bSameFog)
        m_comboFog.SetCurSel(iFirstFog);
      if (bSameHaze)
        m_comboHaze.SetCurSel(iFirstHaze);
    }

    BOOL bDisableForce = FALSE;
    BOOL bDisableFog = FALSE;
    BOOL bDisableHaze = FALSE;
    if (m_comboForceField.GetCount() == 0)
      bDisableForce = TRUE;
    if (m_comboFog.GetCount() == 0)
      bDisableFog = TRUE;
    if (m_comboHaze.GetCount() == 0)
      bDisableHaze = TRUE;
    ...
    FOREACHINDYNAMICCONTAINER(pDoc->m_selSectorSelection, CBrushSector, itbsc) {
      ...
      if (iSector == 0) {
        ...
        if (bsc.GetContentType() != iFirstContent)
          bSameContent = FALSE;
        if (bsc.GetEnvironmentType() != iFirstEnvironment)
          bSameEnvironment = FALSE;
    ...
    if (bSameContent)
      m_comboContentType.SetCurSel(iFirstContent);
    else
      m_comboContentType.SetCurSel(-1);
    if (bSameEnvironment)
      m_comboEnvironmentType.SetCurSel(iFirstEnvironment);
    else
      m_comboEnvironmentType.SetCurSel(-1);
  ...
  if ((pDX->m_bSaveAndValidate != FALSE) && IsWindow(m_SectorAmbientColor.m_hWnd)) {
    ...
    BOOL bColorChanged
      = (colAmbient != m_colLastSectorAmbientColor) || (m_bLastSectorAmbientColorMixed && m_SectorAmbientColor.IsColorValid());
...
BOOL CDlgPgSector::OnIdle(LONG lCount) {
  ...
  BOOL bColorChanged = (colSectorAmbientColor != m_colLastSectorAmbientColor)
                       || (m_bLastSectorAmbientColorMixed && m_SectorAmbientColor.IsColorValid());
