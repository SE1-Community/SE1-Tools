    ...
    for (INDEX iBrush = 0; iBrush < CT_BRUSHES; iBrush++) {
      ...
      if (iBrush == theApp.m_fCurrentTerrainBrush) {
        if (m_pDrawPort->Lock()) {
          ...
          m_pDrawPort->DrawBorder(boxBrush.Min()(1) - 1, boxBrush.Min()(2) - 1, boxBrush.Max()(1) - boxBrush.Min()(1) + 2,
                                  boxBrush.Max()(2) - boxBrush.Min()(2) + 2, colSelected | CT_OPAQUE);
          m_pDrawPort->Unlock();
        }
      }
      ...
      if ((boxBrush & boxPoint) == boxPoint) {
        if (m_pDrawPort->Lock()) {
          ...
          m_pDrawPort->DrawBorder(boxBrush.Min()(1) - 1, boxBrush.Min()(2) - 1, boxBrush.Max()(1) - boxBrush.Min()(1) + 2,
                                  boxBrush.Max()(2) - boxBrush.Min()(2) + 2, C_BLUE | CT_OPAQUE, ulLineType);
          m_pDrawPort->Unlock();
        }
      }
    }
