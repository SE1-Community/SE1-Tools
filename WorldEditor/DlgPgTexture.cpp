...
void CDlgPgTexture::DoDataExchange(CDataExchange* pDX) {
  ...
  if ((pDX->m_bSaveAndValidate == FALSE) && IsWindow(m_comboScroll.m_hWnd)) {
    ...
    for (INDEX iScroll = 0; iScroll < 256; iScroll++) {
      ...
      if (strScrollName != CTString(""))
        m_comboScroll.AddString(CString(strScrollName));
    }
    ...
    for (INDEX iBlend = 0; iBlend < 256; iBlend++) {
      ...
      if (strBlendName != CTString(""))
        m_comboBlend.AddString(CString(strBlendName));
    }
    ...
    if (!bSelectionExists) {
      ...
      FOREACHINDYNAMICCONTAINER(pDoc->m_selPolygonSelection, CBrushPolygon, itbpo) {
        ...
        if (ctPolygons == 0) {
          m_fOffsetU = mdui.mdui_fUOffset;
          m_bOffsetU = TRUE;
          m_fOffsetV = mdui.mdui_fVOffset;
          m_bOffsetV = TRUE;
          m_fRotationU = mdui.mdui_aURotation;
          m_bRotationU = TRUE;
          m_fRotationV = mdui.mdui_aVRotation;
          m_bRotationV = TRUE;
          m_fStretchU = mdui.mdui_fUStretch;
          m_bStretchU = TRUE;
          m_fStretchV = mdui.mdui_fVStretch;
          m_bStretchV = TRUE;
          ...
          if (m_fOffsetU != mdui.mdui_fUOffset)
            m_bOffsetU = FALSE;
          if (m_fOffsetV != mdui.mdui_fVOffset)
            m_bOffsetV = FALSE;
          if (m_fRotationU != mdui.mdui_aURotation)
            m_bRotationU = FALSE;
          if (m_fRotationV != mdui.mdui_aVRotation)
            m_bRotationV = FALSE;
          if (m_fStretchU != mdui.mdui_fUStretch)
            m_bStretchU = FALSE;
          if (m_fStretchV != mdui.mdui_fVStretch)
            m_bStretchV = FALSE;

          if (bpo.bpo_abptTextures[pDoc->m_iTexture].s.bpt_ubScroll != ubFirstScroll)
            bSameScroll = FALSE;
          if (bpo.bpo_abptTextures[pDoc->m_iTexture].s.bpt_ubBlend != ubFirstBlend)
            bSameBlend = FALSE;
        }
        ctPolygons++;
      }
      ...
      if (bSameScroll)
        m_comboScroll.SetCurSel(ubFirstScroll);
      else
        m_comboScroll.SetCurSel(-1);
      if (bSameBlend)
        m_comboBlend.SetCurSel(ubFirstBlend);
      else
        m_comboBlend.SetCurSel(-1);
      // set states of all flags using on and off masks
      if ((ubFlagsOn & BPTF_CLAMPU) && !(ubFlagsOff & BPTF_CLAMPU))
        m_checkClampU.SetCheck(1);
      else if (!(ubFlagsOn & BPTF_CLAMPU) && (ubFlagsOff & BPTF_CLAMPU))
        m_checkClampU.SetCheck(0);
      else
        m_checkClampU.SetCheck(2);
      if ((ubFlagsOn & BPTF_CLAMPV) && !(ubFlagsOff & BPTF_CLAMPV))
        m_checkClampV.SetCheck(1);
      else if (!(ubFlagsOn & BPTF_CLAMPV) && (ubFlagsOff & BPTF_CLAMPV))
        m_checkClampV.SetCheck(0);
      else
        m_checkClampV.SetCheck(2);
      if ((ubFlagsOn & BPTF_REFLECTION) && !(ubFlagsOff & BPTF_REFLECTION))
        m_checkReflective.SetCheck(1);
      else if (!(ubFlagsOn & BPTF_REFLECTION) && (ubFlagsOff & BPTF_REFLECTION))
        m_checkReflective.SetCheck(0);
      else
        m_checkReflective.SetCheck(2);
      if ((ubFlagsOn & BPTF_AFTERSHADOW) && !(ubFlagsOff & BPTF_AFTERSHADOW))
        m_checkAfterShadow.SetCheck(1);
      else if (!(ubFlagsOn & BPTF_AFTERSHADOW) && (ubFlagsOff & BPTF_AFTERSHADOW))
        m_checkAfterShadow.SetCheck(0);
      else
        m_checkAfterShadow.SetCheck(2);
    }
  }
  ...
  if ((pDX->m_bSaveAndValidate != FALSE) && IsWindow(m_comboScroll.m_hWnd)) {
    ...
    if ((m_strTextureFile != DIFFERENT_TEXTURE) && (m_strTextureFile != NO_SELECTION)) {
    ...
    ULONG ulBitsToSet = 0;
    if (m_checkClampU.GetCheck() == 0)
      ulBitsToClear &= ~BPTF_CLAMPU;
    if (m_checkClampU.GetCheck() == 1)
      ulBitsToSet |= BPTF_CLAMPU;
    if (m_checkClampV.GetCheck() == 0)
      ulBitsToClear &= ~BPTF_CLAMPV;
    if (m_checkClampV.GetCheck() == 1)
      ulBitsToSet |= BPTF_CLAMPV;
    if (m_checkReflective.GetCheck() == 0)
      ulBitsToClear &= ~BPTF_REFLECTION;
    if (m_checkReflective.GetCheck() == 1)
      ulBitsToSet |= BPTF_REFLECTION;
    if (m_checkAfterShadow.GetCheck() == 0)
      ulBitsToClear &= ~BPTF_AFTERSHADOW;
    if (m_checkAfterShadow.GetCheck() == 1)
      ulBitsToSet |= BPTF_AFTERSHADOW;
    // for each of the selected polygons
    FOREACHINDYNAMICCONTAINER(pDoc->m_selPolygonSelection, CBrushPolygon, itbpo) {
      CBrushPolygon& bpo = *itbpo;
      ...
      bpo.bpo_abptTextures[pDoc->m_iTexture].bpt_mdMapping.ToUI(mdui);
      if (m_bOffsetU)
        mdui.mdui_fUOffset = m_fOffsetU;
      if (m_bOffsetV)
        mdui.mdui_fVOffset = m_fOffsetV;
      if (m_bRotationU)
        mdui.mdui_aURotation = m_fRotationU;
      if (m_bRotationV)
        mdui.mdui_aVRotation = m_fRotationV;
      if (m_bStretchU)
        mdui.mdui_fUStretch = m_fStretchU;
      if (m_bStretchV)
        mdui.mdui_fVStretch = m_fStretchV;
      bpo.bpo_abptTextures[pDoc->m_iTexture].bpt_mdMapping.FromUI(mdui);

      INDEX iScroll = m_comboScroll.GetCurSel();
      if (iScroll != -1)
        bpo.bpo_abptTextures[pDoc->m_iTexture].s.bpt_ubScroll = (UBYTE)iScroll;
      INDEX iBlend = m_comboBlend.GetCurSel();
      if (iBlend != -1)
        bpo.bpo_abptTextures[pDoc->m_iTexture].s.bpt_ubBlend = (UBYTE)iBlend;
...
BOOL CDlgPgTexture::OnInitDialog() {
  ...
  PIX pixLeft = 0;
  PIX pixTop = 30;
  m_wndViewTexture.Create(NULL, NULL, WS_BORDER | WS_VISIBLE, CRect(pixLeft, pixTop, pixLeft + 64, pixTop + 64), this,
                          IDW_SHOW_TEXTURE);
  DragAcceptFiles();
  if (pDoc == NULL) {
    m_radioTexture = 0;
