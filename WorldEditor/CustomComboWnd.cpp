...
void CCustomComboWnd::OnPaint() {
  ...
  if ((m_pDrawPort != NULL) && (m_pDrawPort->Lock())) {
    ...
    for (INDEX iLine = 0; iLine < m_dcComboLines.Count(); iLine++) {
      ...
      if (bUnderMouse && bSelected)
        colFill = COLOR_SELECTED_UNDER_MOUSE;
      else if (bUnderMouse)
        colFill = COLOR_UNDER_MOUSE;
      else if (bSelected)
        colFill = COLOR_SELECTED;
...
BOOL CCustomComboWnd::Initialize(FLOAT *pfResult, void (*pOnSelect)(INDEX iSelected), PIX pixX, PIX pixY, BOOL bDown /*=FALSE*/) {
  m_pfResult = pfResult;
  ...
  if (IsWindow(m_hWnd)) {
    ...
    BOOL bResult = CreateEx(WS_EX_TOOLWINDOW, NULL, L"Custom combo", WS_CHILD | WS_POPUP | WS_VISIBLE, rectWindow.left,
...
void CCustomComboWnd::RenderOneLine(INDEX iLine, PIXaabbox2D rectLine, CDrawPort *pdp, COLOR colFill) {
  ...
  pdp->Fill(rectLine.Min()(1) - 1, rectLine.Min()(2) - 1, rectLine.Max()(1) - rectLine.Min()(1) + 1,
            rectLine.Max()(2) - rectLine.Min()(2) + 1, colFill);
  if (cl.cl_fnmTexture != "") {
    CTextureObject to;
    try {
