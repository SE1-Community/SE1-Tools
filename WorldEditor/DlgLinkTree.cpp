...
CDlgLinkTree::CDlgLinkTree(CEntity *pen, CPoint pt, BOOL bWhoTargets, BOOL bPropertyNames, CWnd *pParent /*=NULL*/) :
...
void CDlgLinkTree::AddEntityPtrsRecursiv(CEntity *pen, HTREEITEM hParent, CTString strPropertyName) {
  CWorldEditorDoc *pDoc = theApp.GetDocument();
  if (_penAdded.IsMember(pen))
    return;
  // Insert entity's ptrs into directory tree
  HTREEITEM InsertedEntity;
  InsertedEntity = m_ctrTree.InsertItem(0, L"", 0, 0, TVIS_SELECTED, TVIF_STATE, 0, hParent, 0);
...
void CDlgLinkTree::ExpandRecursivly(HTREEITEM pItem, BOOL bExpand, BOOL bNoNextSibling) {
  ...
  if (_iMaxLevel != -1 && GetItemLevel(pItem) >= _iMaxLevel)
    bForceContract = TRUE;
