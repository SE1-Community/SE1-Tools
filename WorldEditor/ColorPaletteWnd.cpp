...
COLOR acol_ColorizePallete[] = {
  C_RED,     C_dGREEN,  C_BLUE,     C_vdCYAN,  C_MAGENTA, C_vdYELLOW, C_ORANGE, C_BROWN, C_PINK,   C_dGRAY,     C_GRAY,
  C_vdGRAY,  C_dRED,    C_lRED,     C_vdGREEN, C_mdGREEN, C_dBLUE,    C_lBLUE,  C_dCYAN, C_vdBLUE, C_vdMAGENTA, C_dMAGENTA,
  C_dYELLOW, C_mdBROWN, C_vdORANGE, C_dORANGE, C_vdBROWN, C_BROWN,    C_dPINK,  C_lPINK, C_mdGRAY, C_vdRED,
};
...
void CColorPaletteWnd::OnPaint() {
  ...
  if ((m_pDrawPort != NULL) && (m_pDrawPort->Lock())) {
    ...
    for (INDEX i = 0; i < 32; i++) {
      ...
      m_pDrawPort->Fill(boxColor.Min()(1) + 1, boxColor.Min()(2) + 1, boxColor.Max()(1) - boxColor.Min()(1) - 2,
                        boxColor.Max()(2) - boxColor.Min()(2) - 2, acol_ColorizePallete[i] | CT_OPAQUE);
      // if we are drawing selected color
      if (i == m_iSelectedColor) {
        m_pDrawPort->DrawLine(boxColor.Min()(1), boxColor.Min()(2), boxColor.Min()(1), boxColor.Max()(2), C_WHITE | CT_OPAQUE,
                              _POINT_);
        m_pDrawPort->DrawLine(boxColor.Min()(1), boxColor.Max()(2), boxColor.Max()(1), boxColor.Max()(2), C_WHITE | CT_OPAQUE,
                              _POINT_);
        m_pDrawPort->DrawLine(boxColor.Max()(1), boxColor.Max()(2), boxColor.Max()(1), boxColor.Min()(2), C_WHITE | CT_OPAQUE,
                              _POINT_);
        m_pDrawPort->DrawLine(boxColor.Max()(1), boxColor.Min()(2), boxColor.Min()(1), boxColor.Min()(2), C_WHITE | CT_OPAQUE,
                              _POINT_);
      }
    }
