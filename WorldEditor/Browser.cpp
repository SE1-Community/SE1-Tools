...
BOOL CBrowser::Create(CWnd *pParentWnd, UINT nIDTemplate, UINT nStyle, UINT nID, BOOL bChange) {
...
CSize CBrowser::CalcDynamicLayout(int nLength, DWORD nMode) {
  ...
  NewBrowserPos = CRect(H_BORDER, V_BORDER + ulNewTreeHeight, csResult.cx - H_BORDER, csResult.cy - V_BORDER);
  m_BrowseWindow.MoveWindow(NewBrowserPos);

  // Finaly we set new virtual tree control size
  NewTreePos = CRect(H_BORDER, V_BORDER, csResult.cx - H_BORDER, V_BORDER + ulNewTreeHeight);
...
void CBrowser::AddDirectoryRecursiv(CVirtualTreeNode *pOneDirectory, HTREEITEM hParent) {
  ...
  InsertedDir = m_TreeCtrl.InsertItem(0, L"", 0, 0, TVIS_SELECTED, TVIF_STATE, 0, hParent, TVI_SORT);
  ...
  m_TreeCtrl.SetItemImage(InsertedDir, pOneDirectory->vtn_itIconType, pOneDirectory->vtn_itIconType + NO_OF_ICONS);
...
void CBrowser::OnDeleteDirectory() {
  ...
  if (::MessageBoxA(this->m_hWnd, "Do You really want to delete directory and all its subdirectories?", "Warning !",
...
CVirtualTreeNode *CBrowser::FindItemDirectory(CVirtualTreeNode *pvtnCurrentDirectory, CTFileName fnItemFileName) {
...
void CBrowser::OnLoadVirtualTree() {
  ...
  if (m_bVirtualTreeChanged) {
    if (::MessageBoxA(this->m_hWnd, "Current virtual directory not saved. Do You want to continue anyway?", "Warning !",
                      MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON1 | MB_TASKMODAL | MB_TOPMOST)
        != IDYES) {
      m_TreeCtrl.SetFocus();
      return;
...
void CBrowser::OnRenameDirectory() {
  if (m_TreeCtrl.GetCount() != 0) {
    ...
    if (dlg.DoModal() == IDOK) {
      ...
      m_TreeCtrl.SetItemImage(pSelectedItem, pVTN->vtn_itIconType, pVTN->vtn_itIconType + NO_OF_ICONS);
...
void CBrowser::OnContextMenu(CWnd *pWnd, CPoint point) {
  ...
  CPoint ptInBrowser = CPoint(point.x - rectBrowser.TopLeft().x, point.y - rectBrowser.TopLeft().y);
...
void CBrowser::OnDumpVt() {
  CTFileName fnName = _EngineGUI.FileRequester("Dump virtual tree as ...", "Text file\0*.txt\0" FILTER_ALL FILTER_END,
                                               "Dump virtual tree directory", "VirtualTrees\\");
  if (fnName == "")
    return;
