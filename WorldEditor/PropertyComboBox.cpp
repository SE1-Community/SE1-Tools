    ...
    for (; pdecDLLClass != NULL; pdecDLLClass = pdecDLLClass->dec_pdecBase) {
      ...
      for (INDEX iProperty = 0; iProperty < pdecDLLClass->dec_ctProperties; iProperty++) {
        ...
        if (epProperty.ep_strName != CTString("")) {
          ...
          CPropertyID *pPropertyID = new CPropertyID(epProperty.ep_strName, epProperty.ep_eptType, &epProperty, pAD);
          // if we should add all of this entity's properties (if this is first entity)
    ...
    FORDELETELIST(CPropertyID, pid_lnNode, m_lhProperties, itProp) {
      ...
      for (; pdecDLLClass != NULL; pdecDLLClass = pdecDLLClass->dec_pdecBase) {
        ...
        for (INDEX iProperty = 0; iProperty < pdecDLLClass->dec_ctProperties; iProperty++) {
          ...
          // create current CPropertyID
          CPropertyID PropertyID = CPropertyID(epProperty.ep_strName, epProperty.ep_eptType, &epProperty, pAD);

          // is this property same as one we are investigating
          if ((strCurrentName == PropertyID.pid_strName) && (eptCurrentType == PropertyID.pid_eptType)) {
...
BOOL CPropertyComboBox::OnIdle(LONG lCount) {
  ...
  if ((m_pLastDoc != pDoc) || ((pDoc == NULL) && (m_pLastDoc != NULL)) || ((pDoc != NULL) && (m_iLastMode != pDoc->m_iMode))
    ...
    if ((pDoc != NULL) && (pDoc->m_iMode == ENTITY_MODE)) {
      ...
      if (pDoc->m_selEntitySelection.Count() != 0) {
        ...
        CPropertyID *ppidSpawnFlags = new CPropertyID("Spawn flags (Alt+Shift+S)", CEntityProperty::EPT_SPAWNFLAGS, NULL, NULL);
        m_lhProperties.AddTail(ppidSpawnFlags->pid_lnNode);
        // -----------------  Add parent entity property
        CPropertyID *ppidParent = new CPropertyID("Parent (Alt+Shift+A)", CEntityProperty::EPT_PARENT, NULL, NULL);
