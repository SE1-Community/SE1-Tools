...
CDlgTreeShortcuts::CDlgTreeShortcuts(CWnd* pParent /*=NULL*/) : CDialog(CDlgTreeShortcuts::IDD, pParent) {
...
void CDlgTreeShortcuts::DoDataExchange(CDataExchange* pDX) {
  ...
  if (pDX->m_bSaveAndValidate == FALSE) {
    ...
    for (INDEX iShortcut = 0; iShortcut < DIRECTORY_SHORTCT_CT; iShortcut++) {
      ...
      HTREEITEM pItem
        = pMainFrame->m_Browser.GetVirtualDirectoryItem(pMainFrame->m_Browser.m_astrVTreeBuffer[iShortcut], iSubDirsCt);
      ASSERT(pItem != NULL);
      // obtain shortcut's virtual tree node
      CVirtualTreeNode* pvtnNode = (CVirtualTreeNode*)pMainFrame->m_Browser.m_TreeCtrl.GetItemData(pItem);
...
BOOL CDlgTreeShortcuts::PreTranslateMessage(MSG* pMsg) {
  if (pMsg->message == WM_KEYDOWN) {
    ...
    if ((pMsg->wParam != VK_CONTROL) && (pMsg->wParam != VK_ESCAPE) && (pMsg->wParam != VK_TAB) && (pMsg->wParam != VK_UP)
        && (pMsg->wParam != VK_DOWN) && (pMsg->wParam != 'W')) {
      if ((pMsg->wParam >= '0') && (pMsg->wParam <= '9')) {
        // remap key ID to number 0-9
        if (pMsg->wParam == '0')
          m_iPressedShortcut = 9;
        else
          m_iPressedShortcut = pMsg->wParam - '1';
      }
      EndDialog(0);
      return TRUE;
    }
