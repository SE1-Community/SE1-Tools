    ...
    if (bSelectionExists) {
      ...
      FOREACHINDYNAMICCONTAINER(pDoc->m_selPolygonSelection, CBrushPolygon, itbpo) {
        ...
        if (iPolygon == 0) {
          ...
          if (itbpo->bpo_bppProperties.bpp_ubSurfaceType != ubFirstFriction)
            bSameFriction = FALSE;
          if (itbpo->bpo_bppProperties.bpp_ubMirrorType != ubFirstMirror)
            bSameMirror = FALSE;
          if (itbpo->bpo_bppProperties.bpp_uwPretenderDistance != uwFirstPretenderDistance)
            m_bPretenderDistance = FALSE;
        }
        iPolygon++;
      }
...
#define SET_TRI_STATE_TO_CTRL(ctrl, flag) \
  if ((ulFlagsOn & flag) && !(ulFlagsOff & flag)) \
    ...
    ctrl.SetCheck(2);
      ...
      if (bSameFriction)
        m_ComboFriction.SetCurSel(ubFirstFriction);
      else
        m_ComboFriction.SetCurSel(-1);
      if (bSameMirror)
        m_ComboMirror.SetCurSel(ubFirstMirror);
      else
        m_ComboMirror.SetCurSel(-1);
      if (m_bPretenderDistance)
        m_fPretenderDistance = uwFirstPretenderDistance;
    }
  ...
  if (pDX->m_bSaveAndValidate != FALSE) {
    ...
    FOREACHINDYNAMICCONTAINER(pDoc->m_selPolygonSelection, CBrushPolygon, itbpo) {
      ...
      if (m_ComboFriction.GetCurSel() != -1)
        itbpo->bpo_bppProperties.bpp_ubSurfaceType = m_ComboFriction.GetCurSel();
...
#define TRI_STATE_CTRL_TO_FLAGS(ctrl, flag, bDiscardShd, bFindShdLayers) \
  if ((ctrl.GetCheck() == 1) && !(itbpo->bpo_ulFlags & flag)) { \
    ...
    if (bFindShdLayers) \
      ...
      BOOL bDetailSet
        = ((ulFlagsBefore & BPOF_DETAILPOLYGON) != (ulFlagsAfter & BPOF_DETAILPOLYGON)) && (ulFlagsAfter & BPOF_DETAILPOLYGON);
      BOOL bDoubleSidedSet
        = ((ulFlagsBefore & BPOF_DOUBLESIDED) != (ulFlagsAfter & BPOF_DOUBLESIDED)) && (ulFlagsAfter & BPOF_DOUBLESIDED);
