...
void CCtrlBrowseFile::OnClicked() {
  ...
  if (fnIntersectingFile.FileExt() == ".mdl") {
    fnChoosedFile
      = _EngineGUI.FileRequester("Choose file", FILTER_MDL FILTER_ALL FILTER_END, KEY_NAME_REQUEST_FILE_DIR,
                                 fnIntersectingFile.FileDir(), fnIntersectingFile.FileName() + fnIntersectingFile.FileExt());
  } else if (fnIntersectingFile.FileExt() == ".tex") {
    fnChoosedFile
      = _EngineGUI.FileRequester("Choose file", FILTER_TEX FILTER_ALL FILTER_END, KEY_NAME_REQUEST_FILE_DIR,
                                 fnIntersectingFile.FileDir(), fnIntersectingFile.FileName() + fnIntersectingFile.FileExt());
  } else if (fnIntersectingFile.FileExt() == ".wav") {
    fnChoosedFile
      = _EngineGUI.FileRequester("Choose file", FILTER_WAV FILTER_ALL FILTER_END, KEY_NAME_REQUEST_FILE_DIR,
                                 fnIntersectingFile.FileDir(), fnIntersectingFile.FileName() + fnIntersectingFile.FileExt());
  } else if (fnIntersectingFile.FileExt() == ".smc") {
    fnChoosedFile
      = _EngineGUI.FileRequester("Choose file", FILTER_SMC FILTER_ALL FILTER_END, KEY_NAME_REQUEST_FILE_DIR,
                                 fnIntersectingFile.FileDir(), fnIntersectingFile.FileName() + fnIntersectingFile.FileExt());
  } else {
    fnChoosedFile
      = _EngineGUI.FileRequester("Choose file", FILTER_ALL FILTER_MDL FILTER_TEX FILTER_WAV FILTER_END, KEY_NAME_REQUEST_FILE_DIR,
                                 fnIntersectingFile.FileDir(), fnIntersectingFile.FileName() + fnIntersectingFile.FileExt());
  }
  ...
  FOREACHINDYNAMICCONTAINER(pDoc->m_selEntitySelection, CEntity, iten) {
    ...
    if (m_bFileNameNoDep) {
      ENTITYPROPERTY(&*iten, penpProperty->ep_slOffset, CTFileNameNoDep) = (CTFileNameNoDep)fnChoosedFile;
    } else {
