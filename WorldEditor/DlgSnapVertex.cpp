...
CDlgSnapVertex::CDlgSnapVertex(CWnd* pParent /*=NULL*/) : CDialog(CDlgSnapVertex::IDD, pParent) {
...
void CDlgSnapVertex::DoDataExchange(CDataExchange* pDX) {
  ...
  if (pDX->m_bSaveAndValidate == FALSE) {
    ...
    if (!bValidY)
      GetDlgItem(IDC_VTX_SNAP_Y)->SetWindowText(L"");
    if (!bValidZ)
      GetDlgItem(IDC_VTX_SNAP_Z)->SetWindowText(L"");
  }
