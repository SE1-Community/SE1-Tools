...
CTString GetPropertyValue(CEntity *pen, CEntityProperty *pepProperty, INDEX &iFormat) {
  ...
  switch (pepProperty->ep_eptType) {
    ...
    case CEntityProperty::EPT_FLOATAABBOX3D: {
      ...
      strResult.PrintF("(%g,%g,%g)-(%g,%g,%g)", box.Min()(1), box.Min()(2), box.Min()(3), box.Max()(1), box.Max()(2),
                       box.Max()(3));
      break;
    }
...
int CALLBACK SortEntities(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort) {
  ...
  if (theApp.m_bInvertClassSort)
    return -iResult;
  else
    return iResult;
}

CDlgBrowseByClass::CDlgBrowseByClass(CWnd *pParent /*=NULL*/) : CDialog(CDlgBrowseByClass::IDD, pParent) {
...
void CDlgBrowseByClass::DoDataExchange(CDataExchange *pDX) {
  ...
  if ((pDX->m_bSaveAndValidate == FALSE) && (::IsWindow(m_listEntities.m_hWnd))) {
    ...
    if (iSelectedItem != -1)
      m_listEntities.EnsureVisible(iSelectedItem, FALSE);
  }
...
void CDlgBrowseByClass::InitializeListColumns(void) {
  ...
  PIX pixPlacement
    = (rectListControl.Width()
       - (pixEntityIndex + pixEntityClassName + pixInstanceName + pixDescription + pixSectorName + pixSpawnFlags + pixDistance))
      / 6;
    ...
    for (; pdecDLLClass != NULL; pdecDLLClass = pdecDLLClass->dec_pdecBase) {
      ...
      for (INDEX iProperty = 0; iProperty < pdecDLLClass->dec_ctProperties; iProperty++) {
        ...
        if (pepProperty->ep_strName != CTString("")) {
          ...
          m_listEntities.InsertColumn(COLUMN_PROPERTY_START + _ctProperties, CString(pepProperty->ep_strName), LVCFMT_LEFT,
                                      pixProperty);
          _ctProperties++;
        }
      }
...
#define MOVE_BUTTON(id, button_collumn) \
  GetDlgItem(id)->MoveWindow(iScreenX - pixButtonOccupies * button_collumn, pixButtonsY, pixButtonOccupies - pixButtonSpacing, \
                             pixButtonHeight);
...
void CDlgBrowseByClass::OnColumnclickEntityList(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  if (theApp.m_iLastClassSortAplied == pNMListView->iSubItem)
    theApp.m_bInvertClassSort = !theApp.m_bInvertClassSort;
  else
    theApp.m_bInvertClassSort = FALSE;
...
BOOL CDlgBrowseByClass::PreTranslateMessage(MSG *pMsg) {
  ...
  if (((pMsg->message == WM_KEYDOWN) || (pMsg->message == WM_SYSKEYDOWN)) && ((int)pMsg->wParam == 'A') && bCtrl) {
    OnSelectAll();
  }
