...
CDlgInfoSheet::CDlgInfoSheet(CWnd* pWndParent) : CPropertySheet(AFX_IDS_APP_TITLE, pWndParent) {
...
BOOL CDlgInfoSheet::OnIdle(LONG lCount) {
  ...
  if (m_InfoMode == IM_NORMAL)
    iLastNormalPage = GetPageIndex(GetActivePage());
  if (m_InfoMode == IM_MAPPING)
    iLastMappingPage = GetPageIndex(GetActivePage());
  ...
  if (pAnyView == NULL) {
    ...
    if ((m_InfoMode != IM_MAPPING) || (ctPages != 3)) {
      HWND hwndActive = ::GetActiveWindow();
      ...
      if (iLastMappingPage < ctPages)
        SetActivePage(iLastMappingPage);
      else
        SetActivePage(0);
      ::SetActiveWindow(hwndActive);
    }
    ...
    AddPage(&m_PgInfoAnim);
    ctPages = GetPageCount();
    if (iLastNormalPage < ctPages)
      SetActivePage(iLastNormalPage);
    else
      SetActivePage(0);
    ::SetActiveWindow(hwndActive);
  }
  ...
  BOOL bRefresh = FALSE;
  if (GetActivePage() == &m_PgInfoAnim)
    bRefresh = TRUE;
  if (m_LastViewUpdated != pAnyView)
    bRefresh = TRUE;
  m_LastViewUpdated = pAnyView;
  if (!theApp.m_chGlobal.IsUpToDate(m_Updateable))
    bRefresh = TRUE;
  if (!pAnyView->m_ModelObject.IsUpToDate(m_Updateable))
    bRefresh = TRUE;
  ...
  if (pMsg->message == WM_KEYDOWN) {
    ...
    BOOL bEditingString
      = (::IsWindow(m_PgAttachingPlacement.m_hWnd)
        ...
        || (::IsWindow(m_PgInfoCollision.m_hWnd) && m_PgInfoCollision.GetDlgItem(IDC_COLLISION_BOX_NAME) == CWnd::GetFocus());
...
void CDlgInfoSheet::CustomSetActivePage(CPropertyPage* pppToActivate) {
  if (GetActivePage() == pppToActivate)
    return;
  INDEX ctPages = GetPageCount();
  for (INDEX iPage = 0; iPage < ctPages; iPage++) {
