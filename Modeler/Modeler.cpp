...
#define INI_READ(strname, def) wcscpy(strIni, theApp.GetProfileString(L"Modeler prefs", CString(strname), CString(def)))
#define GET_FLAG(var) \
  if (wcscmp(strIni, L"YES") == 0) \
    var = TRUE; \
  else \
    var = FALSE;
#define GET_COLOR(var) swscanf(strIni, L"0X%08x", &var);
#define GET_INDEX(var) swscanf(strIni, L"%d", &var);
#define GET_FLOAT(var) swscanf(strIni, L"%f", &var);
...
#define SET_FLOAT(var) \
  ...
  _wcsupr(strIni);
#define INI_WRITE(strname) theApp.WriteProfileString(L"Modeler prefs", CString(strname), strIni)
...
void SetFlagToProfile(CTString strVarName, BOOL bValue) {
  if (bValue)
    theApp.WriteProfileString(L"Modeler prefs", CString(strVarName), L"YES");
  else
    theApp.WriteProfileString(L"Modeler prefs", CString(strVarName), L"NO");
};
...
UINT APIENTRY ModelerFileRequesterHook(HWND hdlg, UINT uiMsg, WPARAM wParam, LPARAM lParam) {
  if (uiMsg == WM_INITDIALOG) {
    ...
    left = (rectMainFrame.right - rectMainFrame.left) / 2 - rectFileReqPos.Width() / 2;
    top = (rectMainFrame.bottom - rectMainFrame.top) / 2 - rectFileReqPos.Height() / 2;
    MoveWindow(GetParent(hdlg), left, top, rectFileReqPos.Width(), rectFileReqPos.Height(), TRUE);
  }
  return 0;
}
...
BOOL CModelerApp::SubInitInstance() {
  ...
  m_pdtModelDocTemplate = pDocTemplate = new CMultiDocTemplate(IDR_MDLDOCTYPE, RUNTIME_CLASS(CModelerDoc),
                                                               RUNTIME_CLASS(CChildFrame), // custom MDI child frame
                                                               RUNTIME_CLASS(CModelerView));
  AddDocTemplate(pDocTemplate);

  m_pdtScriptTemplate = pDocTemplate = new CMultiDocTemplate(IDR_SCRIPTDOCTYPE, RUNTIME_CLASS(CScriptDoc),
                                                             RUNTIME_CLASS(CChildFrame), // custom MDI child frame
  ...
  INDEX iWorkingTexturesCt = theApp.GetProfileInt(L"Modeler prefs", L"Modeler working textures count", -1);
  ...
  INDEX iWorkingPatchesCt = theApp.GetProfileInt(L"Modeler prefs", L"Modeler working patches count", -1);
  char strWPName[128];
  ...
  } catch (char *err_str) { // report error and continue without models
...
void CModelerApp::CreateNewDocument(CTFileName fnRequestedFile) {
  ...
  if (fnRequestedFile.FileExt() != ".scr") {
    if (GetFileAttributesA(_fnmApplicationPath + fnScriptFile) != -1) {
      if (MessageBoxA(m_pMainWnd->m_hWnd,
                      "Script file allready exists, "
    ...
    if (MessageBoxA(m_pMainWnd->m_hWnd,
                    "Model file allready exists, do you want to "
...
void CModelerApp::OnFileNew() {
  ...
  _EngineGUI.FileRequester("Create new model from 3D or script file",
                           "3D objects and scripts\0*.lwo;*.obj;*.3ds;*.scr\0" FILTER_3DOBJ FILTER_LWO FILTER_OBJ FILTER_3DS
                             FILTER_SCR FILTER_ALL FILTER_END,
...
void CAppPrefs::WriteToIniFile() {
  ...
  theApp.WriteProfileInt(L"Modeler prefs", L"Modeler working textures count", iWorkingTexturesCt);
  INDEX iWTCt = 0;
  ...
  theApp.WriteProfileInt(L"Modeler prefs", L"Modeler working patches count", iWorkingPatchesCt);
  INDEX iWPCt = 0;
