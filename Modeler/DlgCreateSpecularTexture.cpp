...
CDlgCreateSpecularTexture::CDlgCreateSpecularTexture(CWnd *pParent /*=NULL*/) : CDialog(CDlgCreateSpecularTexture::IDD, pParent) {
...
void CDlgCreateSpecularTexture::DrawPreview(CDrawPort *pdp, FLOAT fExp) {
  ...
  if ((m_moModel.GetData() == NULL) || (m_moModel.mo_toTexture.GetData() == NULL) || (m_moModel.mo_toSpecular.GetData() == NULL))
...
void CDlgCreateSpecularTexture::RenderPreview(void) {
  ...
  if (m_pPreviewViewPort != NULL)
    m_pPreviewViewPort->SwapBuffers();
}
...
void CDlgCreateSpecularTexture::OnPaint() {
  ...
  if (!m_bCustomWindowsCreated) {
    ...
    m_wndGraph.Create(NULL, NULL, WS_BORDER | WS_VISIBLE, rectGraphWindow, this, IDW_SPECULAR_GRAPH);
    ...
    m_wndPreview.Create(NULL, NULL, WS_BORDER | WS_VISIBLE, rectPreview, this, IDW_SPECULAR_PREVIEW);
...
void CDlgCreateSpecularTexture::OnOK() {
  ...
  CTFileName fnFinal = _EngineGUI.FileRequester("Save texture as ...", FILTER_TEX FILTER_ALL FILTER_END,
                                                "Specular map textures directory", "Textures\\");
  if (fnFinal != "") {
