...
void CDlgPgInfoAttachingPlacement::DoDataExchange(CDataExchange *pDX) {
  ...
  if ((m_iActivePlacement == -1) && (ctPositions != 0))
    m_iActivePlacement = 0;
  ...
  if (!pDX->m_bSaveAndValidate) {
    ...
    if (bAttachmentExists) {
      ...
      char achrLine[256];
      sprintf(achrLine, "center:%d, front:%d, up:%d", pMD->md_aampAttachedPosition[m_iActivePlacement].amp_iCenterVertex,
      ...
      m_bIsVisible = pDoc->m_emEditModel.edm_aamAttachedModels[m_iActivePlacement].am_bVisible != 0;
...
BOOL CDlgPgInfoAttachingPlacement::BrowseAttachement(CAttachedModel *pam) {
  ...
  CTFileName fnModel
    = _EngineGUI.FileRequester("Select model to attach", FILTER_MDL FILTER_END, "Attaching models directory",
                               _fnmApplicationPath + fnOldModel.FileDir(), fnOldModel.FileName() + fnOldModel.FileExt());
  if (fnModel == "")
    return FALSE;
...
void CDlgPgInfoAttachingPlacement::OnRemoveAttachingPlacement() {
  ...
  // and from model data
  CAttachedModelPosition *pampModelPosition = &pMD->md_aampAttachedPosition[m_iActivePlacement];
