...
CModelerView::CModelerView() {
  ...
  CModelerView *pModelerView = DYNAMIC_DOWNCAST(CModelerView, pMainFrame->GetActiveFrame()->GetActiveView());
...
BOOL CModelerView::AssureValidTDI() {
  ...
  if ((bValidTex == FALSE) && (!pDoc->m_emEditModel.edm_WorkingSkins.IsEmpty())) {
    m_ptdiTextureDataInfo = LIST_HEAD(pDoc->m_emEditModel.edm_WorkingSkins, CTextureDataInfo, tdi_ListNode);
    bValidTex = TRUE;
  }
...
void CModelerView::ClearBcg(COLOR color, CDrawPort *pDrawPort) {
  ...
  CTextureObject *ptoValid = (CTextureObject *)theApp.GetValidBcgTexture(m_fnBcgTexture);
  if ((m_IsWinBcgTexture) && (ptoValid != NULL)) {
    PIXaabbox2D screenBox;
    screenBox = PIXaabbox2D(PIX2D(0, 0), PIX2D(pDrawPort->GetWidth(), pDrawPort->GetHeight()));
 ...
 */
static inline void DrawLine(CDrawPort &dp, const FLOAT2D &vPoint0, const FLOAT2D &vPoint1, COLOR color, ULONG ulLineType) {
...
INDEX _iTextLine = 0;
void CModelerView::DrawArrowAndTypeText(CPerspectiveProjection3D &prProjection, const FLOAT3D &v0, const FLOAT3D &v1,
                                        COLOR colColor, CTString strText) {
...
void CModelerView::RenderAxisOfAllAttachments(CPerspectiveProjection3D &prProjection, CPlacement3D &plParent, CModelObject &mo) {
...
void CModelerView::RenderView(CDrawPort *pDrawPort) {
  ...
  if (!m_bMappingMode) {
    ...
    if (m_FloorOn) {
      ...
      rmRenderModel.SetObjectPlacement(CPlacement3D(ANGLE3D(0, 0, 0), FLOAT3D(0.0f, 0.0f, 0.0f)));
      // render floor
    ...
    BeginModelRenderingView(apr, pDrawPort);
    // set light placement
    rmRenderModel.rm_vLightDirection = m_plModelPlacement.pl_PositionVector - plLightPlacement.pl_PositionVector;
    // set color of light
    rmRenderModel.rm_colLight = m_LightColor;
    rmRenderModel.rm_colAmbient = m_colAmbientColor;
    ...
    if ((m_LightModeOn || theApp.m_Preferences.ap_AllwaysSeeLamp) && (theApp.m_pLampModelData != NULL)) {
      // set projection data
      SetProjectionData(prPerspectiveProjection, pDrawPort);
    ...
    if (m_bCollisionMode) {
      ...
      _mrpModelRenderPrefs.SetRenderType(RT_WIRE_ON | RT_HIDDEN_LINES | RT_TEXTURE | RT_SHADING_PHONG);
      // render collision box

      // obtain collision box offset vector
      CPlacement3D plRotatedCollisionBoxOffset = CPlacement3D(bbCollision.Center(), ANGLE3D(0, 0, 0));
  ...
  // mapping mode
    ...
    if ((m_IsMappingBcgTexture) && (pTD != NULL)) {
      if (!m_bTileMappingBCG) {
        ...
        pDrawPort->PutTexture(&m_ModelObject.mo_toTexture, PIXaabbox2D(PIX2D(0, 0), PIX2D(pixSizeI, pixSizeJ)),
                              MEXaabbox2D(MEX2D(mexU0, mexV0), MEX2D(mexU1, mexV1)));
      }
    } else {
      pDrawPort->Fill(boxScreen.Min()(1), boxScreen.Min()(2), boxScreen.Size()(1), boxScreen.Size()(2),
    ...
    if (pMMI->mmpi_ulFlags & MM_PATCHES_VISIBLE) {
      ...
      for (INDEX iMaskBit = 0; iMaskBit < MAX_TEXTUREPATCHES; iMaskBit++) {
        ...
        if ((ptdPatch != NULL) && (m_ModelObject.GetPatchesMask() & ((1UL) << iMaskBit))) {
          ...
          PIX pixPatchUMax
            = (PIX)((mexPatchU + ptdPatch->GetWidth() * pMD->md_mpPatches[iMaskBit].mp_fStretch) * m_MagnifyFactor - m_offx);
          PIX pixPatchVMax
            = (PIX)((mexPatchV + ptdPatch->GetHeight() * pMD->md_mpPatches[iMaskBit].mp_fStretch) * m_MagnifyFactor - m_offy);
          PIXaabbox2D screenBox = PIXaabbox2D(PIX2D(pixPatchUMin, pixPatchVMin), PIX2D(pixPatchUMax, pixPatchVMax));
    ...
    if (m_bRenderMappingInSurfaceColors) {
      for (INDEX iSurface = 0; iSurface < mmi.mmpi_MappingSurfaces.Count(); iSurface++) {
        ...
        pDoc->m_emEditModel.DrawFilledSurface(pDrawPort, pDoc->m_iCurrentMip, iSurface, m_MagnifyFactor, m_offx, m_offy,
                                              ms.ms_colColor, ms.ms_colColor);
      }
    }
    ...
    if (m_ShowAllSurfaces) {
      for (INDEX iSurface = 0; iSurface < mmi.mmpi_MappingSurfaces.Count(); iSurface++) {
        ...
        if (!(ms.ms_ulRenderingFlags & SRF_SELECTED))
          pDoc->m_emEditModel.DrawWireSurface(pDrawPort, pDoc->m_iCurrentMip, iSurface, m_MagnifyFactor, m_offx, m_offy,
    ...
    for (INDEX iSurface = 0; iSurface < mmi.mmpi_MappingSurfaces.Count(); iSurface++) {
      ...
      if (ms.ms_ulRenderingFlags & SRF_SELECTED)
        pDoc->m_emEditModel.DrawWireSurface(pDrawPort, pDoc->m_iCurrentMip, iSurface, m_MagnifyFactor, m_offx, m_offy,
                                            theApp.m_Preferences.ap_MappingActiveSurfaceColor, C_RED);
    }
    // if we should print surface numbers
    if (m_bPrintSurfaceNumbers) {
      pDoc->m_emEditModel.PrintSurfaceNumbers(pDrawPort, theApp.m_pfntFont, pDoc->m_iCurrentMip, m_MagnifyFactor, m_offx, m_offy,
                                              C_BLACK);
    }
  }
}
...
void CModelerView::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  // model view mode
    ...
    // alt colorizes surface
      ...
      m_ModelObject.ColorizePolygon(m_pDrawPort, &prProjection, point.x, point.y, m_iChoosedColor, m_bOnColorMode);
      pDoc->m_emEditModel.edm_md.md_bPreparedForRendering = FALSE;
      pDoc->SetModifiedFlag();
      Invalidate(FALSE);
    }
...
void CModelerView::OnRButtonDown(UINT nFlags, CPoint point) {
  ...
  // if mapping mode
  if (m_bMappingMode) { // RMB + space is used for zooming mapping
    if (bSpace) {
  ...
  // model view mode
  else { // ctrl+shift start mip ranging
    if (bCtrl && bShift) {
...
void CModelerView::OnIdle(void) {
  ...
  BOOL bUpdate
    = !(m_ModelObject.IsPaused() && m_ModelObject.IsUpToDate(m_udViewPicture) && theApp.m_chGlobal.IsUpToDate(m_udViewPicture)
        && theApp.m_chPlacement.IsUpToDate(m_udViewPicture) && (m_ModelObject.GetPatchesMask() == 0));
  CTextureData *pTD = (CTextureData *)m_ModelObject.mo_toTexture.GetData();
  if ((pTD != NULL) && (pTD->td_ctFrames > 1))
    bUpdate = TRUE;
...
void CModelerView::OnContextMenu(CWnd *pWnd, CPoint point) {
  ...
  if (m_bMappingMode) {
    ...
    if ((m_InputAction == IA_CONTEXT_MENU) && (menu.LoadMenu(IDR_MODEL_VIEW_POPUP))) {
      ...
      if (m_RenderPrefs.WireOn())
        pPopup->CheckMenuItem(ID_REND_WIRE_ONOFF, MF_CHECKED);
      if (m_RenderPrefs.HiddenLines())
        pPopup->CheckMenuItem(ID_REND_HIDDEN_LINES, MF_CHECKED);
...
void CModelerView::OnScriptUpdateMipmodels() {
  ...
  if (::MessageBoxA(this->m_hWnd,
                    "Updating mip-models will discard current mip-model mapping "
...
void CModelerView::OnMappingOn() {
  ...
  if (!m_bMappingMode) {
    ...
    if (m_bAutoMipModelingBeforeMapping)
      m_ModelObject.AutoMipModelingOn();
    else
      m_ModelObject.AutoMipModelingOff();
  }
...
void CModelerView::OnTakeScreenShoot() {
  ...
  CTFileName fnSSFileName = _EngineGUI.FileRequester("Select name for screen shot", FILTER_TGA FILTER_END,
                                                     "Take screen shoots directory", "ScreenShots\\", "", NULL, FALSE);
  if (fnSSFileName == "")
    return;
...
void CModelerView::OnStainsInsert() {
  ...
  if (!pDoc->m_emEditModel.EditAddPatch(
        pWP->wp_FileName, MEX2D(mexWidth / 2 - pTD->GetWidth() / 2, mexHeight / 2 - pTD->GetHeight() / 2), iMaskBit)) {
...
BOOL CModelerView::PreTranslateMessage(MSG *pMsg) {
  ...
  if (bViewHasFocus && bAttachmentExists && (pMsg->message == WM_KEYDOWN)
    ...
    if (iClosestVertex != -1) {
      if (pMsg->wParam == '1')
        pAttPg->SetPlacementReferenceVertex(iClosestVertex, -1, -1);
      if (pMsg->wParam == '2')
        pAttPg->SetPlacementReferenceVertex(-1, iClosestVertex, -1);
      if (pMsg->wParam == '3')
        pAttPg->SetPlacementReferenceVertex(-1, -1, iClosestVertex);
    }
  }
  ...
  else if ((pMsg->message == WM_SYSKEYDOWN) || (pMsg->message == WM_KEYDOWN) || (pMsg->message == WM_KEYUP)
           || (pMsg->message == WM_LBUTTONDOWN) || (pMsg->message == WM_RBUTTONDOWN) || (pMsg->message == WM_LBUTTONUP)
           || (pMsg->message == WM_RBUTTONUP) || (bShift || bAlt || bControl || bSpace || bLMB || bRMB)) {
    // in mapping mode
    if (m_bMappingMode) {
      // nothing
...
void CModelerView::OnNextTexture() {
  ...
  m_ptdiTextureDataInfo = (CTextureDataInfo *)pMainFrame->m_SkinComboBox.GetItemDataPtr(iCurrentlySelected + 1);
  // redraw view
  Invalidate(FALSE);
}
...
void CModelerView::OnCreateMipModels() {
  ...
  try {
    ...
    try {
      FOREVER {
        ...
        if (EQUAL_SUB_STR("DIRECTORY")) {
          ...
          FLOATmatrix3D mTran;
          mTran.Diagonal(1.0f);
          sscanf(achrLine, "TRANSFORM %g %g %g %g %g %g %g %g %g", &mTran(1, 1), &mTran(1, 2), &mTran(1, 3), &mTran(2, 1),
    ...
    if (fnFrameFileName != "") {
      ...
      if ((dlgAutoMipModeling.DoModal() != IDOK) || (dlgAutoMipModeling.m_iVerticesToRemove <= 0)
          || (dlgAutoMipModeling.m_iSurfacePreservingFactor < 1) || (dlgAutoMipModeling.m_iSurfacePreservingFactor > 99)) {
        pMainFrame->m_NewProgress.DestroyWindow();
        return;
      }
      // create mip models
      pDoc->m_emEditModel.CreateMipModels_t(objRestFrame, objMipSourceFrame, dlgAutoMipModeling.m_iVerticesToRemove,
                                            dlgAutoMipModeling.m_iSurfacePreservingFactor);
      ...
      for (INDEX iSurface = 1; iSurface < pDoc->m_emEditModel.edm_md.md_MipInfos[0].mmpi_MappingSurfaces.Count(); iSurface++) {
...
INDEX CModelerView::GetClosestVertex(FLOAT3D &vClosestVertex) {
  ...
  // set position of document's model
  prProjection.ObjectPlacementL() = m_plModelPlacement;
  return m_ModelObject.PickVertexIndex(m_pDrawPort, &prProjection, m_MousePosition.x, m_MousePosition.y, vClosestVertex);
}
...
void CModelerView::OnPickVertex() {
  ...
  if (iClosestVertex != -1) {
    char achrLine[256];
    sprintf(achrLine, "Vertex index = %d (%g, %g, %g)", iClosestVertex, vClosestVertex(1), vClosestVertex(2), vClosestVertex(3));
    CMainFrame *pMainFrame = STATIC_DOWNCAST(CMainFrame, AfxGetMainWnd());
...
void CModelerView::OnAddReflectionTexture() {
  CTFileName fnChoosedFile = _EngineGUI.FileRequester("Select reflection texture", FILTER_TEX FILTER_ALL FILTER_END,
                                                      "Reflection textures directory", "Textures\\");
  if (fnChoosedFile == "")
    return;
  try {
...
void CModelerView::OnAddSpecular() {
  CTFileName fnChoosedFile = _EngineGUI.FileRequester("Select specular texture", FILTER_TEX FILTER_ALL FILTER_END,
                                                      "Specularity textures directory", "Textures\\");
  if (fnChoosedFile == "")
    return;
  try {
...
void CModelerView::OnAddBumpTexture() {
  CTFileName fnChoosedFile
    = _EngineGUI.FileRequester("Select bump texture", FILTER_TEX FILTER_ALL FILTER_END, "Bump textures directory", "Textures\\");
  if (fnChoosedFile == "")
    return;
  try {
...
void CModelerView::OnExportSurfaces() {
  CTFileName fnExportFileName = _EngineGUI.FileRequester("Select export file name", FILTER_TXT FILTER_END, "Open model directory",
                                                         "Models\\", "", NULL, FALSE);
  if (fnExportFileName == "")
    return;
...
void CModelerView::OnExportForSkining() {
  ...
  CTFileName fnExportName = _EngineGUI.FileRequester("Select name to export mapping", "Pictures (*.tga)\0*.tga\0" FILTER_END,
  ...
  if (GetFileAttributesA(fnFullPath) != -1) {
    ...
    if (::MessageBoxA(this->m_hWnd, strMsg, "Warning !", MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON1 | MB_SYSTEMMODAL | MB_TOPMOST)
        != IDYES) {
      return;
    }
  }
