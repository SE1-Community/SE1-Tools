...
CDlgMarkLinkedSurfaces::CDlgMarkLinkedSurfaces(CWnd* pParent /*=NULL*/) : CDialog(CDlgMarkLinkedSurfaces::IDD, pParent) {
...
void CDlgMarkLinkedSurfaces::DoDataExchange(CDataExchange* pDX) {
  ...
  if (pDX->m_bSaveAndValidate != FALSE) {
    ...
    for (INDEX iEntry = 0; iEntry < m_listSurfaces.GetCount(); iEntry++) {
      ...
      if (m_listSurfaces.GetCheck(iEntry) == 1)
        pms->ms_ulRenderingFlags |= SRF_SELECTED;
      else
        pms->ms_ulRenderingFlags &= ~SRF_SELECTED;
    }
...
BOOL CDlgMarkLinkedSurfaces::OnInitDialog() {
  ...
  for (INDEX iSurface = 0; iSurface < mmi.mmpi_MappingSurfaces.Count(); iSurface++) {
    ...
    if (ms.ms_ulRenderingFlags & SRF_SELECTED)
      m_listSurfaces.SetCheck(iAddedAs, 1);
    else
      m_listSurfaces.SetCheck(iAddedAs, 0);
  }
  return TRUE;
}
