...
XCALL_(int)
Animation_Handler(long version, GlobalFunc *_global, LWItemMotionHandler *local, void *serverData) {
...
void FindMorphChannels(LWChanGroupID idParentGroup) {
  ...
  for (LWChanGroupID idGroup = _chi->nextGroup(idParentGroup, NULL); idGroup != NULL;
    ...
    for (LWChannelID idChan = _chi->nextChannel(idGroup, NULL); idChan != NULL; idChan = _chi->nextChannel(idGroup, idChan)) {
