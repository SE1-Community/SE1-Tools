...
void CDlgBarTreeView::ShowControlGroup(INDEX iGroup) {
  ...
  switch (iGroup) {
    case GR_PARENT: pdlgCurrent = &m_dlgParent; break;
    case GR_ANIMSET: pdlgCurrent = &m_dlgAnimSet; break;
    case GR_COLISION: pdlgCurrent = &m_dlgColision; break;
    case GR_ALLFRAMES_BBOX: pdlgCurrent = &m_dlgAllFrames; break;
    case GR_LOD: pdlgCurrent = &m_dlgLod; break;
    case GR_BONE: pdlgCurrent = &m_dlgBone; break;
    case GR_TEXTURE: pdlgCurrent = &m_dlgTexture; break;
    case GR_LISTOPTIONS: pdlgCurrent = &m_dlgListOpt; break;
    case GR_SHADERS: pdlgCurrent = &m_dlgShader; break;
    default: pdlgCurrent = NULL; break;
  }
...
void CDlgBarTreeView::ShowSurfaceShader(MeshSurface *pmsrf, MeshLOD *pmlod, MeshInstance *pmshi) {
  ...
  if (iDlgHeight < YPOS)
    m_dlgShader.SetScrollRange(SB_VERT, 1, YPOS - iDlgHeight);
  else
    m_dlgShader.SetScrollRange(SB_VERT, 0, 0);
...
void CDlgBarTreeView::SelItemChanged(HTREEITEM hSelected) {
  ...
  if (ni.ni_iType == NT_MODELINSTANCE)
    pmiSelected = (CModelInstance *)ni.ni_pPtr;
  ...
  if (pmiParent != NULL) {
    ...
    if (pmiParent->mi_psklSkeleton != NULL)
      FillBonesToComboBox(pmiParent->mi_psklSkeleton, 0);
  ...
  switch (ni.ni_iType) {
    ...
    case NT_ANIMSET:
    case NT_MESHLODLIST:
    case NT_SKELETONLODLIST: ShowControlGroup(GR_LISTOPTIONS); break;
    case NT_TEXINSTANCE: {
...
BOOL CDlgBarTreeView::Create(CWnd *pParentWnd, UINT nIDTemplate, UINT nStyle, UINT nID) {
  ...
  if (!CDlgTemplate::Create(pParentWnd, nIDTemplate, nStyle, nID))
    return FALSE;
...
HTREEITEM CDlgBarTreeView::AddModelInst(CModelInstance &mi, CModelInstance *pmiParent, HTREEITEM hParent) {
  ...
  if (hParent == TVI_ROOT) {
    ...
    hItem = m_TreeCtrl.InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_STATE, L"", 0, 0, TVIS_EXPANDED, TVIS_EXPANDED, 0,
                                  hParent, 0);
    m_TreeCtrl.SetItemText(hItem, CString(mi.GetName()));
  } else {
    int iIcon = 0;
    if (mi.mi_fnSourceFile != pmiParent->mi_fnSourceFile)
      iIcon = 8;

    // add child model instance
    hItem = m_TreeCtrl.InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_STATE, L"", iIcon, iIcon, TVIS_EXPANDED, TVIS_EXPANDED,
                                  0, hParent, 0);
...
void CDlgBarTreeView::AddSurfaces(CModelInstance &mi, MeshLOD &mlod, HTREEITEM hParent) {
  ...
  for (INDEX isrf = 0; isrf < ctsrf; isrf++) {
    ...
    if (pShader != NULL)
      strShaderName = pShader->GetName().FileName();
    HTREEITEM hItem
      = m_TreeCtrl.InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE, L"", 11, 11, TVIS_SELECTED, TVIF_STATE, 0, hParent, 0);
    CTString strSurfName;
    strSurfName.PrintF("%s [%d]-[%d]", (const char *)ska_GetStringFromTable(msrf.msrf_iSurfaceID), msrf.msrf_ctVertices,
                       msrf.msrf_aTriangles.Count());
...
void CDlgBarTreeView::AddMeshInstances(CModelInstance &mi, HTREEITEM hParent) {
  ...
  for (INDEX imsh = 0; imsh < ctmsh; imsh++) {
    ...
    for (INDEX itex = 0; itex < cttex; itex++) {
      ...
      HTREEITEM hTexture
        = m_TreeCtrl.InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE, L"", 4, 4, TVIS_SELECTED, TVIF_STATE, 0, hItem, 0);
...
void CDlgBarTreeView::AddColisionBoxes(CModelInstance &mi, HTREEITEM hParent) {
  ...
  for (INDEX icb = 0; icb < ctcb; icb++) {
    ...
    HTREEITEM hColisionBox
      = m_TreeCtrl.InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE, L"", 2, 2, TVIS_SELECTED, TVIF_STATE, 0, hParent, 0);
...
#pragma message(">> Remove AddAllFramesBBox")
  ...
  HTREEITEM hAllFramesBBox
    = m_TreeCtrl.InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE, L"", 6, 6, TVIS_SELECTED, TVIF_STATE, 0, hParent, 0);
...
HTREEITEM CDlgBarTreeView::AddAnimSet(CModelInstance &mi, HTREEITEM hParent) {
  ...
  for (INDEX ias = 0; ias < ctas; ias++) {
    ...
    HTREEITEM hAnimSet
      = m_TreeCtrl.InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE, L"", 9, 9, TVIS_SELECTED, TVIF_STATE, 0, hParent, 0);
    ...
    for (INDEX ian = 0; ian < ctan; ian++) {
      ...
      strAnimName.PrintF("%s [%d]", (const char *)ska_GetStringFromTable(an.an_iID), an.an_iFrames);
      HTREEITEM hAnim
        = m_TreeCtrl.InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE, L"", 9, 9, TVIS_SELECTED, TVIF_STATE, 0, hAnimSet, 0);
      ...
      for (INDEX ibe = 0; ibe < ctbe; ibe++) {
        ...
        if (an.an_bCompresed)
          ctr = be.be_arRotOpt.Count();

        strBoneEnvName.PrintF("%s [%d]-[%d]", (const char *)strBoneName, ctr, be.be_apPos.Count());
        HTREEITEM hBoneEnv
          = m_TreeCtrl.InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE, L"", 9, 9, TVIS_SELECTED, TVIF_STATE, 0, hAnim, 0);
...
void CDlgBarTreeView::UpdateModelInstInfo(CModelInstance *pmi) {
  ...
  if (strRoot != strNewRoot && strRoot.Length() > 0)
    _aSelectItem.PopAll();
