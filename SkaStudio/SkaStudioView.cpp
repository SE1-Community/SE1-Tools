...
void CreateCurrentAnimationList(CModelInstance *pmi, CTString &strAnimations) {
  ...
  for (ial = iFirstAnimList; ial < ctal; ial++) {
    ...
    if (ial + 1 < ctal)
      palListNext = &pmi->mi_aqAnims.aq_Lists[ial + 1];
...
INDEX CSeriousSkaStudioView::TestRayCastHit(CPoint &pt) {
  ...
  if (pDoc != NULL) {
    ...
    if (pmi != NULL) {
      ...
      CPlacement3D plRay;
      apr->RayThroughPoint(FLOAT3D((float)pt.x, m_pdpDrawPort->GetHeight() - (float)pt.y, 0.0f), plRay);
...
void CSeriousSkaStudioView::RenderLightModels(CDrawPort *pdp, CPlacement3D &pl) {
  ...
  if (pDoc != NULL && pDoc->bShowLights && theApp.pmiLight != NULL) {
    ...
    plLightPlacement.pl_PositionVector = FLOAT3D(0.0f, 0.0f, 0.0f);
...
void CSeriousSkaStudioView::RenderView(CDrawPort *pdp) {
  if (theApp.GetDisableRenderRequests() > 0)
    return;
  ...
  if (pDoc->m_ModelInstance != NULL) {
    ...
    /*
    // Render shadow
    FLOATplane3D plShadowPlane = FLOATplane3D(FLOAT3D(0.0f, 1.0f, 0.0f),0);
...
void CSeriousSkaStudioView::OnMouseMove(UINT nFlags, CPoint point) {
  ...
  // if only ctrl
    ...
    if (niSelected.ni_iType == NT_MODELINSTANCE) {
      ...
      plCBMin.pl_OrientationAngle = ANGLE3D(0.0f, 0.0f, 0.0f);
      plCBMin.AbsoluteToRelative(plViewer);

      plCBMax.pl_PositionVector = cb.Max();
      plCBMax.pl_OrientationAngle = ANGLE3D(0.0f, 0.0f, 0.0f);
...
void CSeriousSkaStudioView::OnAddMeshlod() {
  ...
  _EngineGUI.FileRequester("Open ASCII intermediate files", FILTER_MESH, "Open directory", "Models\\", "", &afnMesh);
...
void CSeriousSkaStudioView::OnAddSkeletonlod() {
  ...
  _EngineGUI.FileRequester("Open ASCII intermediate files", FILTER_SKELETON, "Open directory", "Models\\", "", &afnSkeleton);

  // return if no files selected
  if (afnSkeleton.Count() <= 0)
    return;
  ...
  for (INDEX ilod = 0; ilod < ctslod; ilod++) {
    // find current max distance in skeleton
    if ((fMaxDistance - 5) < pSkeleton->skl_aSkeletonLODs[ilod].slod_fMaxDistance)
      fMaxDistance = pSkeleton->skl_aSkeletonLODs[ilod].slod_fMaxDistance;
  }
...
void CSeriousSkaStudioView::OnAddAnimation() {
  ...
  _EngineGUI.FileRequester("Open ASCII intermediate files", FILTER_ANIMATION, "Open directory", "Models\\", "", &afnAnimation);

  // return if no files selected
  if (afnAnimation.Count() <= 0)
    return;
...
void CSeriousSkaStudioView::BrowseTexture(CTString strTextureDir) {
  ...
  _EngineGUI.FileRequester("Open texture files", FILTER_TEXTURE, (char *)(const char *)strTextureDir, strTextureDir, "",
                           &afnTexture);

  // return if no files selected
  if (afnTexture.Count() <= 0)
    return;
...
void CSeriousSkaStudioView::OnUpdateAnimLoop(CCmdUI *pCmdUI) {
  CSeriousSkaStudioDoc *pDoc = theApp.GetDocument();
  if (pDoc != NULL)
    pCmdUI->SetCheck(pDoc->bAnimLoop);
}
...
void CSeriousSkaStudioView::OnUpdateAutoMiping(CCmdUI *pCmdUI) {
  CSeriousSkaStudioDoc *pDoc = theApp.GetDocument();
  if (pDoc != NULL)
    pCmdUI->SetCheck(pDoc->bAutoMiping);
}
...
void CSeriousSkaStudioView::OnUpdateShowGround(CCmdUI *pCmdUI) {
  CSeriousSkaStudioDoc *pDoc = theApp.GetDocument();
  if (pDoc != NULL)
    pCmdUI->SetCheck(pDoc->bShowGround);
}

void CSeriousSkaStudioView::OnShowSkeleton() {
  if (RM_GetFlags() & RMF_SHOWSKELETON)
    RM_RemoveFlag(RMF_SHOWSKELETON);
  else
    RM_AddFlag(RMF_SHOWSKELETON);
}
...
void CSeriousSkaStudioView::OnShowActiveSkeleton() {
  if (RM_GetFlags() & RMF_SHOWACTIVEBONES)
    RM_RemoveFlag(RMF_SHOWACTIVEBONES);
  else
    RM_AddFlag(RMF_SHOWACTIVEBONES);
}
...
void CSeriousSkaStudioView::OnShowTexture() {
  if (RM_GetFlags() & RMF_SHOWTEXTURE)
    RM_RemoveFlag(RMF_SHOWTEXTURE);
  else
    RM_AddFlag(RMF_SHOWTEXTURE);
}

void CSeriousSkaStudioView::OnShowNormals() {
  if (RM_GetFlags() & RMF_SHOWNORMALS)
    RM_RemoveFlag(RMF_SHOWNORMALS);
  else
    RM_AddFlag(RMF_SHOWNORMALS);
}
...
void CSeriousSkaStudioView::OnShowWireframe() {
  if (RM_GetFlags() & RMF_WIREFRAME)
    RM_RemoveFlag(RMF_WIREFRAME);
  else
    RM_AddFlag(RMF_WIREFRAME);
}
...
void CSeriousSkaStudioView::OnUpdateShowColision(CCmdUI *pCmdUI) {
  CSeriousSkaStudioDoc *pDoc = theApp.GetDocument();
  if (pDoc != NULL)
    pCmdUI->SetCheck(pDoc->bShowColisionBox);
}
...
void CSeriousSkaStudioView::OnUpdateShowAllFramesBbox(CCmdUI *pCmdUI) {
  CSeriousSkaStudioDoc *pDoc = theApp.GetDocument();
  if (pDoc != NULL)
    pCmdUI->SetCheck(pDoc->bShowAllFramesBBox);
}
...
void CSeriousSkaStudioView::OnUpdateShowLights(CCmdUI *pCmdUI) {
  CSeriousSkaStudioDoc *pDoc = theApp.GetDocument();
  if (pDoc != NULL)
    pCmdUI->SetCheck(pDoc->bShowLights);
}
